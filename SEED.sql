...
-- Bu script örnek kullanıcılar, içerikler ve piyasa verileri ekler.
-- ══════════════════════════════════════════════════════════════════════════════

-- ─── 1. SEED USERS (auth.users + profiles) ───────────────────────────────────
-- Not: Supabase SQL Editor service role ile çalışır, auth.users'a insert yapabilir.
...
INSERT INTO auth.users (
  ...
  ('00000000-0000-0000-0000-000000000000', u7, 'authenticated', 'authenticated',
   ...
   '{"provider":"email","providers":["email"]}', '{"full_name":"Demo Kullanıcı"}', NOW(), NOW())
...
-- ─── 4. SIGNALS ──────────────────────────────────────────────────────────────
INSERT INTO signals (
  id, creator_id, asset_id, direction, confidence,
  entry_price, target_price, stop_loss,
  timeframe, rationale,
  copies_count, likes_count,
  is_active, created_at
) VALUES
  (gen_random_uuid(), '11111111-1111-1111-1111-111111111111', 'BTC', 'BUY', 5,
   63800, 72000, 60500, 'Orta Vade',
   'Yükselen kanal içinde sağlam destek. RSI aşırı satım bölgesinden çıkıyor. 4h kapanış güçlü. Hash rate ATH seviyesinde.',
   340, 1240, true, NOW() - INTERVAL '14 minutes'),

  (gen_random_uuid(), '22222222-2222-2222-2222-222222222222', 'THYAO', 'BUY', 4,
   248, 285, 235, 'Kısa Vade',
   'Kargo gelirlerinde güçlü artış beklentisi. Teknik olarak omuz-baş-omuz formasyonu tamamlanmak üzere. Hacim artışı olumlu.',
   218, 876, true, NOW() - INTERVAL '2 hours'),

  (gen_random_uuid(), '33333333-3333-3333-3333-333333333333', 'XAU', 'BUY', 5,
   2340, 2480, 2290, 'Uzun Vade',
   'Enflasyon beklentileri ve merkez bankası alımları altını destekliyor. Fed pivot yaklaşıyor. Jeopolitik risk primi devam ediyor.',
   567, 2100, true, NOW() - INTERVAL '1 hour'),

  (gen_random_uuid(), '44444444-4444-4444-4444-444444444444', 'NVDA', 'BUY', 4,
   875, 1050, 820, 'Orta Vade',
   'AI chip talebi rekor kırıyor. Q4 beklentileri pozitif. Teknik olarak kupa-sap formasyonu tamamlanıyor.',
   289, 988, true, NOW() - INTERVAL '3 hours'),

  (gen_random_uuid(), '55555555-5555-5555-5555-555555555555', 'SOL', 'BUY', 4,
   138, 165, 128, 'Kısa Vade',
   'DeFi hacmi rekor kırıyor. Teknik olarak sıkışma formasyonu yukarı kırıldı. TVL artışı güçlü.',
   289, 988, true, NOW() - INTERVAL '5 hours'),

  (gen_random_uuid(), '33333333-3333-3333-3333-333333333333', 'ETH', 'SELL', 3,
   3200, 2850, 3380, 'Kısa Vade',
   'Direnç bölgesinde momentum zayıflıyor. MACD negatif kesişim yaptı. Risk/ödül oranı favorable.',
   124, 432, true, NOW() - INTERVAL '4 hours'),

  (gen_random_uuid(), '22222222-2222-2222-2222-222222222222', 'AAPL', 'BUY', 3,
   189, 215, 180, 'Uzun Vade',
   'Apple AI stratejisi yatırımcıları heyecanlandırdı. iPhone 16 satışları beklentilerin üzerinde. Büyük yatırımcılar pozisyon artırıyor.',
...
INSERT INTO asset_prices (asset_id, price, change_percent, volume, market_cap, spark, updated_at) VALUES
  ('BTC',    67420,  2.40, '$28.5B',  '$1.3T',  ARRAY[60,62,61,65,67,66,68,67,69,70], NOW()),
  ('ETH',    3240,   1.80, '$14.2B',  '$389B',  ARRAY[30,31,32,31,33,32,34,33,35,36], NOW()),
  ('BNB',    415,   -0.90, '$1.8B',   '$62B',   ARRAY[42,41,43,42,41,40,41,42,41,40], NOW()),
  ('SOL',    178,    4.20, '$5.1B',   '$82B',   ARRAY[16,17,17,18,17,18,19,18,19,20], NOW()),
  ('XRP',    0.63,  -1.20, '$2.4B',   '$34B',   ARRAY[6,6,6,7,6,6,6,7,6,6],          NOW()),
  ('AAPL',   189.5,  0.80, '$4.2B',   '$2.9T',  ARRAY[18,19,19,19,19,19,19,19,20,20], NOW()),
  ('NVDA',   875,    3.10, '$18.5B',  '$2.1T',  ARRAY[82,83,85,84,86,86,87,87,88,89], NOW()),
  ('TSLA',   242,   -2.10, '$8.9B',   '$770B',  ARRAY[25,24,24,25,25,24,24,24,24,23], NOW()),
  ('MSFT',   415,    1.20, '$6.1B',   '$3.1T',  ARRAY[40,40,41,41,41,41,41,41,42,42], NOW()),
  ('AMZN',   185,    0.90, '$5.8B',   '$1.9T',  ARRAY[18,18,19,19,19,19,19,19,19,19], NOW()),
  ('XAU',    2345,   0.40, '$142B',   '-',      ARRAY[233,234,234,234,234,235,235,235,235,236], NOW()),
  ('XAG',    27.85, -0.61, '$12B',    '-',      ARRAY[28,28,28,27,27,27,27,27,27,27], NOW()),
  ('WTI',    78.4,  -0.60, '$28B',    '-',      ARRAY[79,78,79,78,79,78,79,78,79,78], NOW()),
  ('USDTRY', 32.14,  0.20, '-',       '-',      ARRAY[32,32,32,32,32,32,32,32,32,32], NOW()),
  ('EURTRY', 34.82,  0.30, '-',       '-',      ARRAY[35,35,35,35,35,35,35,35,35,35], NOW()),
  ('EURUSD', 1.0845,-0.22, '$45B',    '-',      ARRAY[11,11,11,11,11,11,11,11,11,11], NOW())
...
INSERT INTO portfolio_holdings (user_id, asset_id, symbol, name, quantity, avg_cost, created_at) VALUES
  ...
  ('33333333-3333-3333-3333-333333333333', 'XAU',  'XAU',  'Altın',      2.0, 2100,  NOW() - INTERVAL '60 days')
ON CONFLICT DO NOTHING;
...
BEGIN
  ...
  RAISE NOTICE '📧 Diğer hesaplar: cryptoguru@marketly.app / Marketly123!';
END $$;
