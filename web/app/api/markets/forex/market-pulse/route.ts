...
import { fetchForexMarketPulse } from "@/features/markets/forex/lib/build-forex-market-pulse";
...
export async function GET(request: Request) {
  ...
  const payload = await fetchForexMarketPulse(
    symbol,
    Number.isFinite(fallbackChange) ? fallbackChange : 0,
  );

  if (!payload) {
    return NextResponse.json({ error: "FX pulse verisi bulunamadı" }, { status: 404 });
  }
