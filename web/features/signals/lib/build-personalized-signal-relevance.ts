...
/** Live + mock ortak kişiselleştirilmiş sinyal sıralaması (mock import yok). */
export function buildPersonalizedSignalRelevance(
  ...
  const headline =
    union.size > 0
      ? `Takip / portföy kesişimi · ${rows.length} çağrı`
      : warmGraph
        ? "İlgi grafiğine göre önerilen çağrılar"
        : "Öne çıkan aktif çağrılar";
