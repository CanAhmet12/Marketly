...
export function SignalDiscussionPanel({ pack, symbol, assetHref, signalsHref, locked, onNavigate, signalId }: Props) {
  ...
  return (
    <div className="ms-metric-block p-[var(--sp-3)]">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex flex-wrap justify-end gap-1">
          <Link
            href={assetHref}
            onClick={onNavigate}
            className="rounded-full border border-[var(--ms-border-hairline)] bg-[color-mix(in_srgb,var(--color-text)_4%,transparent)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-secondary)] hover:border-[var(--color-primary)]/35"
          >
            Varlık
          </Link>
          <Link
      ...
      <p className="mt-1 text-[10px] font-medium text-[var(--color-meta)]">
        Katılım hissi · %{sentimentSplit.bullPct} boğa · %{sentimentSplit.bearPct} ayı
      </p>

      <div className="mt-2 flex flex-wrap gap-1">
        <span className="inline-flex items-center rounded-md bg-[color-mix(in_srgb,var(--color-rise)_12%,transparent)] px-1.5 py-0.5 text-[10px] font-bold tabular-nums text-emerald-800 ring-1 ring-emerald-500/20">
          ↑ {reactions.bullish}
        </span>
        <span className="inline-flex items-center rounded-md bg-[color-mix(in_srgb,var(--color-fall)_12%,transparent)] px-1.5 py-0.5 text-[10px] font-bold tabular-nums text-red-800 ring-1 ring-red-500/20">
          ↓ {reactions.bearish}
        </span>
        <span className="inline-flex items-center rounded-md bg-[color-mix(in_srgb,var(--color-text)_6%,transparent)] px-1.5 py-0.5 text-[10px] font-semibold tabular-nums text-[var(--color-text-secondary)] ring-1 ring-[var(--ms-border-hairline)]">
          İzleme {reactions.tracking}
        </span>
        <span className="inline-flex items-center rounded-md bg-[color-mix(in_srgb,var(--color-text)_6%,transparent)] px-1.5 py-0.5 text-[10px] font-semibold tabular-nums text-[var(--color-text-secondary)] ring-1 ring-[var(--ms-border-hairline)]">
          Kopya {reactions.copied}
        </span>
        <span className="inline-flex items-center rounded-md bg-[color-mix(in_srgb,var(--color-fall)_8%,transparent)] px-1.5 py-0.5 text-[10px] font-semibold tabular-nums text-[var(--color-text-secondary)] ring-1 ring-[var(--ms-border-hairline)]">
      ...
      <ul className="m-0 mt-3 max-h-[280px] list-none space-y-2 overflow-y-auto overflow-x-hidden p-0">
        {previewEntries.map((e) => (
          <li key={e.id} className="min-w-0 border-b border-[color-mix(in_srgb,var(--ms-border-hairline)_75%,transparent)] pb-2 last:border-0 last:pb-0">
            <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-semibold text-[var(--color-meta)]">
              <span className="rounded bg-[color-mix(in_srgb,var(--color-text)_7%,transparent)] px-1 py-px text-[9px] font-bold uppercase tracking-wide text-[var(--color-text-secondary)]">
                {threadEntryBadge(e.kind)}
              </span>
              {e.role === "creator" ? (
                <span className="rounded bg-[color-mix(in_srgb,var(--color-primary)_12%,transparent)] px-1 py-px text-[9px] font-bold text-[var(--color-primary-dark)]">
                  Üretici
                </span>
              ) : null}
              ...
              {e.sentiment ? (
                <span className="shrink-0 rounded px-1 py-px text-[9px] font-bold uppercase text-[var(--color-text-secondary)]">
                  {e.sentiment === "bullish" ? "Boğa" : e.sentiment === "bearish" ? "Ayı" : "Nötr"}
                </span>
              ) : null}
            </div>
            {e.quoteSnippet ? (
              <p className="mt-0.5 line-clamp-1 border-l-2 border-[var(--color-primary)]/40 pl-2 text-[10px] font-medium italic text-[var(--color-text-secondary)]">
                “{e.quoteSnippet}”
              </p>
            ) : null}
            {locked ? (
      ...
      {linked.length ? (
        <div className="mt-3 border-t border-[color-mix(in_srgb,var(--ms-border-hairline)_80%,transparent)] pt-2">
          <p className="text-[10px] font-bold uppercase text-[var(--color-meta)]">Bağlı gönderiler</p>
          <ul className="m-0 mt-1.5 list-none space-y-1.5 p-0">
            {linked.map((l) => (
              <li key={l.post_id}>
                ...
                <span className="mt-0.5 block text-[10px] text-[var(--color-meta)]">{l.heat}</span>
              </li>
            ))}
          </ul>
        </div>
