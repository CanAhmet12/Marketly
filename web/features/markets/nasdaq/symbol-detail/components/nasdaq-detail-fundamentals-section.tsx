...
const SLICE_COLORS: Record<NasdaqFundamentalsSlice["key"], string> = {
  stock: "#22d3ee",
  production: "#06b6d4",
  seasonal: "#38bdf8",
};

function severityClass(severity: NasdaqFundamentalsInsight["severity"]): string {
...
function buildValuationRing(slices: NasdaqFundamentalsSlice[]): string {
  let cursor = 0;
  const stops: string[] = [];
...
export function NasdaqDetailFundamentalsSection({ symbol, name }: Props) {
  const sym = symbol.trim().toUpperCase();
  const query = useNasdaqDetailFundamentals(sym, name);
  const { data } = query;
  ...
  if (surface.showInitialSkeleton) {
    return (
      <section className="cdr-section cdr-tokenomics-section cdr-tokenomics-section--inline" data-zone="fundamentals">
        <DetailSectionHead seriesKicker="Temel" label="Temel Analiz" accent="peak" />
      ...
      <div className="cdr-tokenomics__canvas">
        <div className="cdr-tokenomics__bento" role="list">
          ...
          <div className="cdr-tokenomics__tile cdr-tokenomics__tile--lock" role="listitem">
            <span className="cdr-tokenomics__tile-accent" aria-hidden />
            <div className="cdr-tokenomics__tile-body">
              <span className="cdr-tokenomics__tile-k">EPS</span>
              <span className="cdr-tokenomics__tile-v">{data.eps}</span>
              <span className="cdr-tokenomics__tile-sub">{data.epsSub}</span>
            </div>
          </div>
          ...
          <div className="cdr-tokenomics__tile cdr-tokenomics__tile--pressure cdr-tokenomics__tile--pressure-mid" role="listitem">
            <span className="cdr-tokenomics__tile-accent" aria-hidden />
            <div className="cdr-tokenomics__tile-body">
              <span className="cdr-tokenomics__tile-k">Gelir büyümesi</span>
        ...
        <div className="cdr-tokenomics__body">
          ...
          <div className="cdr-tokenomics__facts">
            ...
            <div className="cdr-tokenomics__fact cdr-tokenomics__fact--max">
              <span className="cdr-tokenomics__fact-k">Analist hedef</span>
