...
import type { NasdaqPeerRow } from "@/features/markets/nasdaq/lib/nasdaq-detail-types";
import { useNasdaqDetailPeerComparison } from "@/features/markets/nasdaq/symbol-detail/hooks/use-nasdaq-peer-comparison";
...
const SPREAD_RING_COLORS = ["#06b6d4", "#22d3ee", "#38bdf8", "#3b82f6", "#64748b"];
...
function buildSpreadRing(rows: NasdaqPeerRow[]): { gradient: string; leaderPct: string } {
...
function rowAccentClass(row: NasdaqPeerRow): string {
...
export function NasdaqDetailPeerComparisonSection({ symbol }: Props) {
  const sym = symbol.trim().toUpperCase();
  const query = useNasdaqDetailPeerComparison(sym);
  const { data } = query;
  ...
  if (surface.showInitialSkeleton) {
    return (
      <section className="cdr-section cdr-markets-section cdr-markets-section--inline" data-zone="peers">
        <DetailSectionHead seriesKicker="Sektör" label="Sektör / Peer Karşılaştırması" accent="teal" />
  ...
  return (
    ...
    >
      <DetailSectionHead
        ...
        label="Sektör / Peer Karşılaştırması"
        accent="teal"
        trailing={
        ...
        <div className="cdr-markets__bento" role="list">
          ...
          <div className="cdr-markets__tile cdr-markets__tile--count" role="listitem">
            ...
            <div className="cdr-markets__tile-body">
              ...
              <span className="cdr-markets__tile-sub">Aynı sektör / endeks</span>
            </div>
          </div>
        </div>
