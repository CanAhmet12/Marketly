...
function FlowCol({
  ...
  return (
    <div className="cc-intel-col nq-intel-col">
        ...
        {rows.length === 0 ? (
          ...
          rows.slice(0, 5).map((row, index) => (
            <li key={`${title}-${row.symbol}`}>
              <Link href={`/markets/${encodeURIComponent(row.symbol)}`} className="cc-intel-row nq-intel-row">
                <span className="cc-intel-row-num">{index + 1}</span>
                <span className="nq-intel-symbol-badge">{row.symbol.slice(0, 2)}</span>
...
export function NasdaqIntelDeck({ movers }: Props) {
  const cols = [
  ...
  return (
    <section className="cc-section cc-intel-board nq-intel-board" role="region" aria-label="NASDAQ piyasa istihbaratı">
      <div className="cc-zone-label cc-zone-label--flush">
        Piyasa istihbaratı
        <span className="cc-intel-board-sub">Tech akış özeti</span>
      </div>
