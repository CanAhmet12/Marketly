...
function dotColor(v: number) {
  ...
  if (v > 0)  return "rgba(6,182,212,0.5)";
  if (v < -2) return "#ef4444";
  if (v < 0)  return "rgba(239,68,68,0.5)";
  return "#64748b";
}

type ColProps = { title: string; rows: NasdaqMoverItem[]; showVolume?: boolean };
...
export function NasdaqTopMovers({ movers }: Props) {
  return (
    <div className="cc-movers-v2 cc-section" role="region" aria-label="Big Tech hareketlileri">
      <div className="cc-movers-v2-header">Big Tech Hareketlileri</div>
