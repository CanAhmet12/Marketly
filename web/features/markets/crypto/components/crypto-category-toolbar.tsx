...
const ECOSYSTEM_LINKS = [
  ...
  { href: "/economic-calendar", label: "Takvim" },
...
export function CryptoCategoryToolbar({ assets }: Props) {
  const router = useRouter();
  const search = useMarketsSearch();
  const trending = useMemo(() => trendingFromAssets(assets, 8), [assets]);

  return (
    <div className="cc-toolbar cc-section" role="region" aria-label="Kripto araç çubuğu">
      ...
      <nav className="cc-ecosystem-nav" aria-label="Kripto ekosistem bağlantıları">
        {ECOSYSTEM_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="cc-ecosystem-nav__link">
            {link.label}
          </Link>
        ))}
      </nav>
