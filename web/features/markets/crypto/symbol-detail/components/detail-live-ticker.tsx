...
import {
  buildStableCryptoTickers,
  type StableTickerItem,
} from "@/features/markets/crypto/symbol-detail/lib/detail-stable-ticker";
...
function TickerRow({
  ...
  return (
    <div className="cdr-live-ticker__row" aria-hidden={ariaHidden || undefined}>
      {items.map((t) => {
        const isActive = t.symbol.trim().toUpperCase() === active;
        return (
          <Link
            key={`${keyPrefix}${t.id}`}
            href={t.href}
          ...
          >
            <span className="cdr-live-ticker__sym">{t.symbol}</span>
...
export function DetailLiveTickerStrip({ assets, activeSymbol }: Props) {
  const orderRef = useRef<string[] | null>(null);

  const tickers = useMemo(() => {
    const { tickers: next, order } = buildStableCryptoTickers(assets, orderRef.current);
  ...
  return (
    <nav className="cdr-live-ticker" aria-label="Kripto canlı piyasa şeridi" data-marquee={marquee || undefined}>
