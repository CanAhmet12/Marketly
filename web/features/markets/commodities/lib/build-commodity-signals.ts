...
export function buildCommoditySignalsPayload(
  ...
  const topAssets = [...pool]
    ...
    .slice(0, 6)
    .map((a) => {
      const name = commodityDisplayLabel(a.symbol, a.name);
      return {
  ...
  return {
    ...
    bullPct: avgBull,
    bearPct: 100 - avgBull,
    marketBiasLabel: avgBull >= 58 ? "Alış ağırlıklı" : avgBull <= 42 ? "Satış ağırlıklı" : "Nötr emtia",
    topAssets,
  };
}
