...
export function CommoditiesCategoryToolbar({ assets }: Props) {
  const router = useRouter();
  const search = useMarketsSearch();
  const trending = useMemo(() => trendingFromAssets(assets, 8), [assets]);

  return (
    <div className="cc-toolbar cm-toolbar cc-section" role="region" aria-label="Emtia araç çubuğu">
      ...
      <nav className="cc-ecosystem-nav cm-ecosystem-nav" aria-label="Emtia ekosistem bağlantıları">
        {ECOSYSTEM_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="cc-ecosystem-nav__link cm-ecosystem-nav__link">
            {link.label}
          </Link>
        ))}
      </nav>
