...
import type { CommoditySignalStripPayload } from "@/features/markets/commodities/types";
...
type Props = {
  ...
  /** Tasarım önizlemesi veya mock modunda katalog satırları */
  useMockCatalog?: boolean;
};

function CommoditySignalRailSkeleton() {
  return (
    <div className="cc-signal-rail-scroll cc-signal-rail-scroll--loading cm-signal-rail-scroll--loading" aria-hidden>
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="cc-signal-rail-skeleton-card cm-signal-rail-skeleton-card" />
      ))}
    </div>
  );
}

export function CommoditiesSignalStrip({ signals, useMockCatalog = false }: Props) {
  ...
  const items = useMemo(() => {
    ...
    const commodityRows = pool
      .filter((row) => resolveSignalAssetCategory(row) === "commodity")
      .sort((a, b) => {
    ...
    return commodityRows.map(mapFeedRowToLiveCardItem);
  ...
  return (
    <SignalsEngagementProvider>
      <section className="cc-section cc-signal-rail cm-signal-rail" role="region" aria-label="Emtia sinyal istihbaratı">
        <div className="cc-signal-rail-head">
          ...
          <div className="cc-signal-rail-meta">
            ...
            <div className="cc-signal-rail-bias-bar cm-signal-rail-bias-bar" aria-hidden>
              <div
                className="cc-signal-rail-bias-fill cm-signal-rail-bias-fill"
        ...
        {showSkeleton ? (
          <CommoditySignalRailSkeleton />
        ) : items.length > 0 ? (
          <HScroll className="cc-signal-rail-scroll dvr-hscroll--sig-rail cm-signal-rail-scroll">
            {items.map((item, index) => (
              <div key={item.id} className="cc-signal-rail-item cm-signal-rail-item shrink-0">
                <div className="cc-signal-rail-card-wrap cm-signal-rail-card-wrap">
                  <SignalsLiveRailCard
                    item={item}
                    index={index}
