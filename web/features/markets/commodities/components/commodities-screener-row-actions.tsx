...
export function CommoditiesScreenerRowActions({ symbol, name, watched, pending, onToggleWatch }: Props) {
  ...
  return (
    <div className="cc-screener-actions" onClick={(e) => e.stopPropagation()}>
      ...
      <Link href={signalsHref} className="cc-screener-btn cc-screener-btn--pill" aria-label={`${name} sinyalleri`}>
        Sinyal
      </Link>
      <Link
        href={detailHref}
        className="cc-screener-btn cc-screener-btn--pill cc-screener-btn--primary"
        aria-label={`${name} detayı`}
      >
        Detay
      </Link>
    </div>
