...
export type CommodityRelatedCard = {
  symbol: string;
  name: string;
  price: number;
  change: number;
...
export function buildCommodityRelatedCards(
  liveAssets: readonly MarketAssetView[],
  excludeSym: string,
  limit: number,
  frozenOrder: string[] | null,
): { cards: CommodityRelatedCard[]; order: string[] } {
  const sym = excludeSym.trim().toUpperCase();
  const pool = filterCommodityAssets(liveAssets).filter((a) => a.symbol.trim().toUpperCase() !== sym);
