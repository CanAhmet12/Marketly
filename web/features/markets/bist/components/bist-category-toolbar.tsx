...
export function BistCategoryToolbar({ assets }: Props) {
  const router = useRouter();
  const search = useMarketsSearch();
  const trending = useMemo(() => trendingFromAssets(assets, 8), [assets]);

  return (
    <div className="cc-toolbar bc-toolbar cc-section" role="region" aria-label="BIST araç çubuğu">
      ...
      <nav className="cc-ecosystem-nav bc-ecosystem-nav" aria-label="BIST ekosistem bağlantıları">
        {ECOSYSTEM_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="cc-ecosystem-nav__link bc-ecosystem-nav__link">
            {link.label}
          </Link>
        ))}
      </nav>
