...
import {
  bistTreemapDominanceSummary,
  buildBistTreemapCells,
} from "@/features/markets/bist/lib/build-bist-treemap";
import type { BistScreenerAsset, BistTreemapCell, BistTreemapPayload } from "@/features/markets/bist/types";
import { cn } from "@/lib/cn";

type Props = {
  screenerAssets: readonly BistScreenerAsset[];
...
function stripTone(symbol: string, change: number) {
  if (symbol === "THYAO" || symbol === "GARAN") return "bc-tmap-seg--mega";
  if (symbol === "ASELS" || symbol === "KCHOL") return "bc-tmap-seg--large";
...
function tileTone(symbol: string, change: number) {
  if (symbol === "THYAO" || symbol === "GARAN") return "bc-tmap-tile--mega";
  if (symbol === "ASELS" || symbol === "KCHOL") return "bc-tmap-tile--large";
  if (change >= 0.3) return "cc-tmap-tile--up";
  if (change <= -0.3) return "cc-tmap-tile--down";
  return "";
}

function WeightTile({ cell }: { cell: BistTreemapCell }) {
  const isUp = cell.changePct >= 0;

  return (
    <Link
      href={`/markets/${encodeURIComponent(cell.symbol)}`}
      className={cn("cc-tmap-tile bc-tmap-tile", tileTone(cell.symbol, cell.changePct))}
      aria-label={`${cell.name} ${cell.weightPct.toFixed(1)}% ağırlık ${signed(cell.changePct)}`}
    >
      <div className="cc-tmap-tile-head">
        <span className="bc-tmap-symbol-badge">{cell.symbol.slice(0, 2)}</span>
...
export function BistSectorTreemap({ screenerAssets, treemap }: Props) {
  const cells = useMemo(
    () => treemap?.cells ?? buildBistTreemapCells(screenerAssets),
    [screenerAssets, treemap],
  );

  const { top3, top3Weight } = useMemo(() => bistTreemapDominanceSummary(cells), [cells]);

  if (cells.length < 4) return null;

  return (
    <section className="cc-section cc-tmap bc-tmap" role="region" aria-label="BIST piyasa değeri ağırlığı">
        ...
        <div className="cc-tmap-kpis">
          ...
          {top3.map((cell) => (
            <div key={cell.symbol} className="cc-tmap-kpi cc-tmap-kpi--chip">
              <span className="bc-tmap-kpi-symbol">{cell.symbol}</span>
