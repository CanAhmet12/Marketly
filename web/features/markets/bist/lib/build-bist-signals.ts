...
export function buildBistSignalsPayload(
  ...
  const pool = hasSignals ? withSignals : assets.filter((a) => !a.symbol.startsWith("XU"));
  ...
  return {
    ...
    bullPct: avgBull,
    bearPct: 100 - avgBull,
    marketBiasLabel: avgBull >= 58 ? "Alış ağırlıklı" : avgBull <= 42 ? "Satış ağırlıklı" : "Nötr BIST",
    topAssets,
  };
}
