...
function ForexSignalRailSkeleton() {
  return (
    <div className="cc-signal-rail-scroll cc-signal-rail-scroll--loading fc-signal-rail-scroll--loading" aria-hidden>
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="cc-signal-rail-skeleton-card fc-signal-rail-skeleton-card" />
      ))}
    </div>
  );
}

export function ForexSignalStrip({ signals }: Props) {
  const router = useRouter();
  const mockOn = isMockDataEnabled();
  const { rows: liveRows, isLoading } = useSignalsCatalog({ scope: "live", sort: "trending" });

  const items = useMemo(() => {
    const pool = mockOn ? getSignalsRepository().getFeedRows() : liveRows;
    const priority = new Set(signals.topAssets.map((asset) => asset.symbol));

    const forexRows = pool
      .filter((row) => resolveSignalAssetCategory(row) === "forex")
      .sort((a, b) => {
    ...
    return forexRows.map(mapFeedRowToLiveCardItem);
  }, [liveRows, mockOn, signals.topAssets]);

  if (!signals.totalActiveSignals && items.length === 0) return null;

  const showSkeleton = !mockOn && isLoading && items.length === 0;

  return (
    <SignalsEngagementProvider>
      <section className="cc-section cc-signal-rail fc-signal-rail" role="region" aria-label="Forex sinyal istihbaratı">
        <div className="cc-signal-rail-head">
          ...
          <div className="cc-signal-rail-meta">
            ...
            <div className="cc-signal-rail-bias-bar fc-signal-rail-bias-bar" aria-hidden>
              <div
                className="cc-signal-rail-bias-fill fc-signal-rail-bias-fill"
        ...
        {showSkeleton ? (
          <ForexSignalRailSkeleton />
        ) : items.length > 0 ? (
          <HScroll className="cc-signal-rail-scroll dvr-hscroll--sig-rail fc-signal-rail-scroll">
            {items.map((item, index) => (
              <div key={item.id} className="cc-signal-rail-item fc-signal-rail-item shrink-0">
                <div className="cc-signal-rail-card-wrap fc-signal-rail-card-wrap">
                  <SignalsLiveRailCard
                    item={item}
                    index={index}
