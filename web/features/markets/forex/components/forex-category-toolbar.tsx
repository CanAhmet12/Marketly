...
export function ForexCategoryToolbar({ assets }: Props) {
  const router = useRouter();
  const search = useMarketsSearch();
  const trending = useMemo(() => trendingFromAssets(assets, 8), [assets]);

  return (
    <div className="cc-toolbar fc-toolbar cc-section" role="region" aria-label="Forex araç çubuğu">
      ...
      <nav className="cc-ecosystem-nav fc-ecosystem-nav" aria-label="Forex ekosistem bağlantıları">
        {ECOSYSTEM_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="cc-ecosystem-nav__link fc-ecosystem-nav__link">
            {link.label}
          </Link>
        ))}
      </nav>
