...
export function ForexScreenerRowActions({ symbol, pair, watched, pending, onToggleWatch }: Props) {
  ...
  return (
    <div className="cc-screener-actions" onClick={(e) => e.stopPropagation()}>
      ...
      <Link href={signalsHref} className="cc-screener-btn cc-screener-btn--pill" aria-label={`${pair} sinyalleri`}>
        Sinyal
      </Link>
      <Link
        href={detailHref}
        className="cc-screener-btn cc-screener-btn--pill cc-screener-btn--primary"
        aria-label={`${pair} detayı`}
      >
        Detay
      </Link>
    </div>
