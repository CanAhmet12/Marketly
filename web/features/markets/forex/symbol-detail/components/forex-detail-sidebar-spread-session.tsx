...
function sessionPillText(status: "open" | "closed" | "pre"): string {
  ...
  if (status === "pre") return "Yakında";
  return "Kapalı";
}

function SpreadSessionInner({ symbol }: Props) {
  const sym = symbol.trim().toUpperCase().replace("/", "");
  const query = useForexDetailSpreadSession(sym);
  const { data } = query;
  const surface = useDetailSectionSurface(query);

  if (surface.showInitialSkeleton) {
    return (
      <section className="cdr-section cdr-sidebar-block cdr-sidebar-block--orderbook fx-sidebar-block--spread" data-zone="spread">
  ...
  return (
    <section
      className="cdr-section cdr-sidebar-block cdr-sidebar-block--orderbook fx-sidebar-block--spread"
      data-zone="spread"
    ...
    >
      ...
      <div className="cdr-orderbook__meta">
        <div className="cdr-orderbook__meta-item cdr-orderbook__meta-item--spread">
          ...
          <span className="cdr-orderbook__meta-v">
            {formatPipCount(Math.round(data.spread.spreadPips))}
      ...
      <div className="fx-spread-ladder" aria-label="Seans spread merdiveni">
        <div className="fx-spread-ladder__head" aria-hidden>
          <span>Seans</span>
          <span>Spread</span>
        </div>
        {data.rows.map((row) => (
          <div key={`${row.venueName}-${row.pair}`} className="fx-spread-ladder__row">
            <div className="fx-spread-ladder__id">
              <span className="fx-spread-ladder__name">{row.venueName}</span>
              <span className="fx-spread-ladder__pair">{row.pair}</span>
            </div>
            <div className="fx-spread-ladder__bar-wrap">
              <span
                className="fx-spread-ladder__bar"
                style={{ width: `${row.depthPct}%` }}
                aria-hidden
              />
            </div>
            <span className="fx-spread-ladder__bps">{formatPipCount(Math.round(row.spreadPips))}</span>
          </div>
        ))}
      </div>
    </section>
...
export const ForexDetailSidebarSpreadSession = memo(SpreadSessionInner);
