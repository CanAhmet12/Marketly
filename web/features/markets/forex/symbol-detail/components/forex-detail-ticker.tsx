...
function TickerRow({
  ...
  const active = activeSymbol.trim().toUpperCase().replace("/", "");

  return (
    <div className="cdr-live-ticker__row" aria-hidden={ariaHidden || undefined}>
      {items.map((t) => {
        const sym = t.symbol.trim().toUpperCase().replace("/", "");
        ...
        return (
          ...
          >
            <span className="cdr-live-ticker__sym">{t.pair}</span>
...
export function ForexDetailTickerStrip({ assets, activeSymbol }: Props) {
  ...
  return (
    <nav className="cdr-live-ticker" aria-label="Forex canlı piyasa şeridi" data-marquee={marquee || undefined}>
